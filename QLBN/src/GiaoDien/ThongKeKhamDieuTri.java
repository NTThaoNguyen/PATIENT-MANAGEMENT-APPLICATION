...
public class ThongKeKhamDieuTri extends javax.swing.JFrame {

    private Vector ds = null;

    public ThongKeKhamDieuTri() {
        initComponents();
    ...
    private void layDS(Date ngayBD, Date ngayKT) {
        ...
        try {
            PreparedStatement ps = ketNoi.prepareStatement("SELECT BN.HOTEN, KDT.NGAYKHAM, KDT.KETQUA\n"
                    + "FROM (SELECT IDBA, IDBN FROM BENHAN) BA, (SELECT IDBN, HO + ' ' + TEN AS HOTEN FROM BENHNHAN) BN, \n"
                    + "(SELECT * FROM KHAMDIEUTRI WHERE NGAYKHAM BETWEEN '" + d1 + "' AND '" + d2 + "') KDT\n"
                    + "WHERE BA.IDBA = KDT.IDBA AND BA.IDBN = BN.IDBN");
    ...
    private void initComponents() {
        ...
        jLabel1.setText("THỐNG KÊ KHÁM ĐIỀU TRỊ");
...
//        }
    ...
    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
        ...
        if (kt == true) {
            layDS(ngayBD, ngayKT);
            duongDan = "E:/Thống kê khám điều trị từ ngày " + d1 + " đến ngày " + d2 + ".xlsx";
    ...
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new ThongKeKhamDieuTri().setVisible(true);
            }
        });
    }
